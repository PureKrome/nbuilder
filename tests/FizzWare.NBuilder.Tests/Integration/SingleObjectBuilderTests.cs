...
namespace FizzWare.NBuilder.Tests.Integration
{
    ...
    public class SingleObjectBuilderTests
    {
        ...
        {
            var builderSetup = new BuilderSettings();
            var ex = Should.Throw<TypeCreationException>(
                () => new Builder(builderSetup).CreateNew<MyAbstractClass>().Build(), "Cannot build an abstract class");
        ...
        {
            var builderSetup = new BuilderSettings();
            var ex = Assert.Throws<TypeCreationException>(() => new Builder(builderSetup).CreateNew<IMyInterface>()
                .Build());
        ...
        {
            var builderSetup = new BuilderSettings();
            Assert.Throws<TypeCreationException>(() => { new Builder(builderSetup).CreateNew<IProduct>().Build(); });
        }
        ...
        {
            var builderSetup = new BuilderSettings();
            Assert.Throws<TypeCreationException>(() => { new Builder(builderSetup).CreateNew<ChuckNorris>().Build(); });
        }
    }
}